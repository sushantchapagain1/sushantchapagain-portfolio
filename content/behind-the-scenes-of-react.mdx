...
reconciler(fiber) and the `diffing` algorithm. Now lets cover the process after Virtual DOM is created, On the inital render of the application,
...
render. remember i said each component instance hold its own state props but internally fiber is the one that takes care of it since there is each fiber for each component
instance props, state, side effects, hooks all live here.
